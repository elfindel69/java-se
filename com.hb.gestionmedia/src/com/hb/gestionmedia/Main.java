...
public class Main {
	 ...
	 */
	private static void creerEmprunt() {
		//initialisation de l'objet
		...
		boolean testAdh = mediatheque.testIdAdherent(idAdherent);
		...
		boolean testMedia = mediatheque.testIdMedia(idMedia);
		if(testMedia) {
			//setter id media
		...
		String strEmprunt = sc.nextLine();
		LocalDate empruntDate  = LocalDate.parse(strEmprunt);
		emprunt.setDateEmprunt(empruntDate);
		
		//saisie date de retour
		System.out.println("entrer une date de retour (format yyyy-MM-dd): ");
		String strRetour = sc.nextLine();
		LocalDate retourDate  = LocalDate.parse(strRetour);
		emprunt.setDateRetour(retourDate);
		
		if(testAdh && testMedia) {
